...
<body>
    <?php if($verDetalles != ""):?>
        <h2>Descripcion: <br><?= descripcion($verDetalles, $descripcionExtendida) ?></h2>
        <h2>Codigo de referencia: <br><?= codigoReferencia($verDetalles, $descripcionExtendida) ?></h2>
                ...
                <form action="index.php" method="post">
                    <tr>
                        ...
                        <?php foreach ($datos as $nombre => $dato): ?>
                            <td
                            <?php if(($nombre == "nombre") && esFavorito($juego)):?>
                                style="font-weight: bold; color: red;"
                            <?php endif;?>
                            ><?= $dato ?></td>
                            <?php if($nombre == "descripcion"):?>
                                <td><input type="submit" name="detalles" value="Ver detalles"></td>
